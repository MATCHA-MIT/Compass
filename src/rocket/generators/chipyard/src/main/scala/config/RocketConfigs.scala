...
class MyRocketNaiveConfig extends Config(
  new freechips.rocketchip.subsystem.WithMarkFor("naive") ++
  new MyRocketBaseConfig)

class MyRocketAutoSandboxConfig extends Config(
  new freechips.rocketchip.subsystem.WithMarkFor("auto_sandbox") ++
  new MyRocketBaseConfig)

class MyRocketCelliftConfig extends Config(
  new freechips.rocketchip.subsystem.WithMarkFor("cellift") ++
  new MyRocketBaseConfig)

class MyRocketBaseConfig extends Config(
  new freechips.rocketchip.subsystem.MyWithICache(nSets=2, nWays=1) ++
  new freechips.rocketchip.subsystem.MyWithDCache(nSets=2, nWays=1) ++
  
  new freechips.rocketchip.subsystem.WithNoSimulationTimeout ++  // remove non-synthesizable code
  new freechips.rocketchip.subsystem.WithoutTLMonitors ++        // remove non-synthesizable code

  new freechips.rocketchip.subsystem.MyWithoutVM ++              // no virtual memory
  new freechips.rocketchip.subsystem.MyWithoutAtomics ++         // no atomics unit (This unit generate invalid taint propagation but I do not understand it)
  new freechips.rocketchip.subsystem.MyWithoutMulDiv ++
