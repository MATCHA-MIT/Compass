...
class ICacheModule(outer: ICache) extends LazyModuleImp(outer)
  ...
  if (tileParams.markFor=="auto_sandbox") {
    iftConfig(this, "Naive, PerWord")  // 2-15
  }
 ...
 * */
  ...
  val my_icache_valid = Wire(Bool())
  val my_icache_addr  = Wire(UInt())
  val my_imem_valid   = Wire(Bool())
  val my_imem_addr    = Wire(UInt())

  my_icache_valid := io.req.fire
  my_icache_addr  := io.req.bits.addr
  my_imem_valid   := tl_out.a.valid
  my_imem_addr    := (refill_paddr >> blockOffBits) << blockOffBits

  dontTouch(my_icache_valid)
  dontTouch(my_icache_addr)
