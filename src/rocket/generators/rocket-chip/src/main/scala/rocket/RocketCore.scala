...
class Rocket(tile: RocketTile)(implicit p: Parameters) extends CoreModule()(p)
  ...
  if (tileParams.markFor=="auto_sandbox") {
  ...
  val div = Module(new MulDiv(if (pipelinedMul) mulDivParams.copy(mulUnroll = 0) else mulDivParams, width = xLen, aluFn = aluFn, markFor=tileParams.markFor))
  ...
  when (!ctrl_killd) {
    ...
    for (i <- 0 until id_raddr.size) {
      ...
      if (tileParams.markFor=="auto_sandbox") {
        iftConfig(ex_reg_rs_lsb(i), "Refined")  // 2-1
        iftConfig(ex_reg_rs_bypass(i), "Refined")  // 5-1
      }
  ...
  // STEP: Shadow Logic.
  val my_mem_reg_memaddr = Reg(UInt())
  val my_wb_reg_memaddr  = Reg(UInt())

  my_mem_reg_memaddr := io.dmem.req.bits.addr
  my_wb_reg_memaddr  := my_mem_reg_memaddr


  // STEP: Extract Signals.
  val my_commit_valid    = Wire(Bool())
  val my_commit_pc       = Wire(UInt())
  val my_commit_inst     = Wire(UInt())
  val my_commit_memvalid = Wire(Bool())
  val my_commit_memaddr  = Wire(UInt())
  val my_commit_wen      = Wire(Bool())
  val my_commit_data     = Wire(UInt())
  val my_csr_cmd         = Wire(UInt())


  my_commit_valid    := wb_valid
  my_commit_pc       := wb_reg_pc
  my_commit_inst     := wb_reg_inst
  my_commit_memvalid := wb_ctrl.mem
  my_commit_memaddr  := my_wb_reg_memaddr
  my_commit_wen      := rf_wen && rf_waddr=/=0.U
  my_commit_data     := rf_wdata
  my_csr_cmd         := id_ctrl.csr

  dontTouch(my_commit_valid)
  dontTouch(my_commit_pc)
  dontTouch(my_commit_inst)
  dontTouch(my_commit_memvalid)
  dontTouch(my_commit_memaddr)
  dontTouch(my_commit_wen)
  dontTouch(my_commit_data)
  ...
  if (tileParams.markFor=="auto_sandbox") {
    iftConfig(ex_pc_valid, "Refined")  // 1-4
    iftConfig(ex_ctrl.sel_alu1, "Refined")  // 2-3
    iftConfig(ex_ctrl.sel_alu2, "Refined")  // 2-4
    iftConfig(dmem_resp_valid, "Refined")  // 2-12
    iftConfig(io.dmem.resp.bits.tag, "Refined")  // 2-12
    iftConfig(id_ctrl.amo, "Refined")  // 2-23
    iftConfig(id_amo_rl, "Refined")  // 2-23
    iftConfig(id_ctrl.fence_i, "Refined")  // 2-23
    iftConfig(id_reg_fence, "Refined")  // 2-23
    iftConfig(id_ctrl.mem, "Refined")  // 2-23
    iftConfig(id_ctrl.rocc, "Refined")  // 2-23
    iftConfig(mem_wrong_npc, "Refined")  // 2-24
    iftConfig(rf_wen, "Refined")  // 2-29
    iftConfig(rf_waddr, "Refined")  // 2-29
    iftConfig(id_raddr1, "Refined")  // 2-29
    iftConfig(wb_ctrl.rocc, "Refined")  // 2-30
    iftConfig(wb_ctrl.mem, "Refined")  // 2-33
    iftConfig(mem_reg_xcpt, "Refined")  // 3-1
    iftConfig(mem_reg_valid, "Refined")  // 3-1
    iftConfig(take_pc_wb, "Refined")  // 3-6
    iftConfig(mem_ctrl.fp, "Refined")  // 7-1
    iftConfig(mem_ctrl.wxd, "Refined")  // 7-1
    iftConfig(mem_pc_valid, "Refined")  // 9-1
    iftConfig(replay_wb, "Refined")  // 10-1
    iftConfig(ibuf.io.inst(0).bits.replay, "Refined")  // 10-5
    iftConfig(take_pc_mem_wb, "Refined")  // 10-5
    iftConfig(ctrl_stalld, "Refined")  // 10-5
    iftConfig(csr.io.interrupt, "Refined")  // 10-5
    iftConfig(ex_pc_valid, "Refined")  // 11-1
    iftConfig(mem_reg_flush_pipe, "Refined")  // 11-1
    iftConfig(wb_ctrl.csr, "Refined")  // 11-2
    iftConfig(ex_ctrl.rxs2, "Refined")  // 11-3
    iftConfig(ex_ctrl.mem, "Refined")  // 11-3
    iftConfig(ex_ctrl.rocc, "Refined")  // 11-3
    iftConfig(csr.io.eret, "Refined")  // 14-1
    iftConfig(wb_xcpt, "Refined")  // 14-1
  }
