...
class CSRFile(
  ...
  if (tileParams.markFor=="auto_sandbox") {
    iftConfig(this, "Naive, PerWord")  // 2-9
  }
  ...
  when (csr_wen) {
    ...
    when (decoded_addr(CSRs.misa)) {
      ...
      val my_temp = Wire(Bool())
      my_temp := !usingCompressed.B || !io.pc(1) || wdata('c' - 'a')
      if (tileParams.markFor=="auto_sandbox") {
        iftConfig(my_temp, "Refined")  // 2-35
      }
    ...
    if (usingHypervisor) {
      ...
      when (decoded_addr(CSRs.vsatp)) {
        ...
        val mode_ok = Wire(Bool())
        mode_ok := new_vsatp.mode.isOneOf(satp_valid_modes.map(_.U))
        when (mode_ok) {
    ...
    if (reg_pmp.nonEmpty) for (((pmp, next), i) <- (reg_pmp zip (reg_pmp.tail :+ reg_pmp.last)).zipWithIndex) {
      ...
      if (tileParams.markFor=="auto_sandbox") {
        iftConfig(next.cfg.a, "Refined")  // 2-38
        iftConfig(next.cfg.l, "Refined")  // 2-38
      }
  ...
  if (tileParams.markFor=="auto_sandbox") {
    iftConfig(csr_wen, "Refined")  // 2-14
    iftConfig(io.rw.addr, "Refined")  // 2-14
    iftConfig(reg_mstatus.v, "Refined")  // 2-14
    iftConfig(reg_bp(0).control.dmode, "Refined")  // 2-34
    iftConfig(reg_debug, "Refined")  // 2-34
    iftConfig(reg_pmp(0).cfg.l, "Refined")  // 2-40
    iftConfig(reg_dcsr.step, "Refined")  // 10-2
    iftConfig(io.rw.cmd, "Refined")  // 10-7
    iftConfig(io.exception, "Refined")  // 10-8
    iftConfig(insn_call, "Refined")  // 10-8
    iftConfig(insn_break, "Refined")  // 10-8
  }
