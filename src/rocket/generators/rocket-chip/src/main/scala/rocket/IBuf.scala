...
class IBuf(implicit p: Parameters) extends CoreModule {
  ...
  if (tileParams.markFor=="auto_sandbox") {
    iftConfig(this, "Naive, PerWord")  // 10-6
    iftConfig(nBufValid, "Refined")  // 12-1
    iftConfig(nReady, "Refined")  // 12-1
    iftConfig(io.imem.bits.btb.taken, "Refined")  // 12-1
    iftConfig(io.imem.bits.btb.bridx, "Refined")  // 12-1
    iftConfig(io.imem.bits.pc, "Refined")  // 12-1
  }
}
