...
class FrontendModule(outer: Frontend) extends LazyModuleImp(outer)
  ...
  if (tileParams.markFor=="auto_sandbox") {
    iftConfig(this, "Naive, PerWord")  // 2-13
  }
  ...
  if (tileParams.markFor=="auto_sandbox") {
    iftConfig(fq, "Naive, PerWord")  // 10-3
  }
  ...
  if (usingBTB) {
    ...
    val my_btb_io_resp_valid = Wire(Bool())
    my_btb_io_resp_valid := btb.io.resp.valid
    if (tileParams.markFor=="auto_sandbox") {
      iftConfig(my_btb_io_resp_valid, "Refined")  // 3-7
    }
    ...
    when (!s2_replay) {
      ...
      s2_btb_resp_valid := my_btb_io_resp_valid
      s2_btb_resp_bits := btb.io.resp.bits
    }
    when (my_btb_io_resp_valid && btb.io.resp.bits.taken) {
    ...
    def scanInsns(idx: Int, prevValid: Bool, prevBits: UInt, prevTaken: Bool): Bool = {
      ...
      if (tileParams.markFor=="auto_sandbox") {
        iftConfig(predictBranch, "Refined")  // 3-3
        iftConfig(predictJump, "Refined")  // 3-3
        iftConfig(predictReturn, "Refined")  // 3-5
      }
  ...
  if (tileParams.markFor=="auto_sandbox") {
    iftConfig(io.cpu.req.valid, "Refined")  // 1-3
    iftConfig(fq.io.mask, "Refined")  // 1-3
    iftConfig(s1_valid, "Refined")  // 1-3
    iftConfig(s2_valid, "Refined")  // 1-3
    iftConfig(s2_speculative, "Refined")  // 2-36
    iftConfig(s2_tlb_resp.cacheable, "Refined")  // 2-36
    iftConfig(io.ptw.customCSRs.csrs(0).value, "Refined")  // 2-36
    iftConfig(s2_can_speculatively_refill, "Refined")  // 2-37
    iftConfig(s2_xcpt, "Refined")  // 2-37
    iftConfig(s2_btb_taken, "Refined")  // 3-3
    iftConfig(s2_btb_resp_valid, "Refined")  // 3-4
    iftConfig(s2_tlb_resp.miss, "Refined")  // 10-4
    iftConfig(icache.io.resp.valid, "Refined")  // 10-4
    iftConfig(s2_kill_speculative_tlb_refill, "Refined")  // 10-4
    iftConfig(icache.io.s2_kill, "Refined")  // 10-4
  }
