...
class DCacheDataArray(implicit p: Parameters) extends L1HellaCacheModule()(p) {
  ...
  if (tileParams.markFor=="auto_sandbox") {
    iftConfig(this, "Refined, PerWord")
  }
  ...
  val rdata = for ((array , i) <- data_arrays.zipWithIndex) yield {
    ...
    // OPT: Guard taint bit earlier.
    // val data = Mux(
    //   valid && !io.req.bits.write,
    //   array.read(addr, valid && !io.req.bits.write),
    //   VecInit(Seq.fill(nWays * (subWordBits / eccBits))(0.U(encBits.W)))
    // )
...
class DCacheModule(outer: DCache) extends HellaCacheModule(outer) {
  ...
  if (tileParams.markFor=="auto_sandbox") {
    iftConfig(metaArb, "Refined, PerWord")  // 2-19
    iftConfig(dataArb, "Refined, PerWord")  // 8-1
  }
  ...
  val my_dcache_valid = Wire(Bool())
  val my_dcache_addr  = Wire(UInt())
  val my_dmem_valid   = Wire(Bool())
  val my_dmem_addr    = Wire(UInt())

  my_dcache_valid := io.cpu.req.fire
  my_dcache_addr  := io.cpu.req.bits.addr
  my_dmem_valid   := tl_out_a.valid
  my_dmem_addr    := (s2_req.addr >> idxLSB) << idxLSB

  dontTouch(my_dcache_valid)
  dontTouch(my_dcache_addr)
  ...
  if (tileParams.markFor=="auto_sandbox") {
    iftConfig(s2_valid, "Refined")  // 2-5
    iftConfig(s2_valid_no_xcpt, "Refined")  // 2-6
    iftConfig(s2_valid_masked, "Refined")  // 2-6
    iftConfig(s2_req.cmd, "Refined")  // 2-6
    iftConfig(s1_xcpt_valid, "Refined")  // 2-7
    iftConfig(s1_valid_not_nacked, "Refined")  // 2-10
    iftConfig(s1_flush_valid, "Refined")  // 2-10
    iftConfig(s2_probe, "Refined")  // 2-11
    iftConfig(io.cpu.req.valid, "Refined")  // 2-16
    iftConfig(tl_out.d.valid, "Refined")  // 2-17
    iftConfig(s0_clk_en, "Refined")  // 2-21
    iftConfig(tlb_port.req.valid, "Refined")  // 2-21
    iftConfig(s2_dont_nack_misc, "Refined")  // 2-22
    iftConfig(s2_valid_hit_pre_data_ecc_and_waw, "Refined")  // 2-22
    iftConfig(io.cpu.req.bits.cmd, "Refined")  // 2-25
    iftConfig(release_state, "Refined")  // 2-26
    iftConfig(s2_victimize, "Refined")  // 2-26
    iftConfig(s2_valid_miss, "Refined")  // 2-27
    iftConfig(s2_uncached, "Refined")  // 2-27
    iftConfig(s2_valid_cached_miss, "Refined")  // 2-28 This is traced back from tlXbar_8 module
    iftConfig(release_ack_wait, "Refined")  // 2-28
    iftConfig(s2_req.addr, "Refined")  // 2-28
    iftConfig(release_ack_addr, "Refined")  // 2-28
    iftConfig(s1_valid, "Refined")  // 2-31
    iftConfig(inWriteback, "Refined")  // 4-1
    iftConfig(tl_out.d.ready, "Refined")  // 4-1
    iftConfig(grantIsUncachedData, "Refined")  // 4-1
    iftConfig(advance_pstore1, "Refined")  // 13-1
    iftConfig(pstore1_valid_likely, "Refined")  // 13-2
    iftConfig(s1_vaddr, "Refined")  // 13-2
    iftConfig(pstore1_addr, "Refined")  // 13-2
    iftConfig(s1_write, "Refined")  // 15-1
  }
