...
class DatPath(implicit val p: Parameters, val conf: SodorCoreParams) extends Module
{
   ...
   my_commit_valid    := !io.ctl.stall && exe_reg_valid
   my_commit_pc       := exe_reg_pc
   my_commit_inst     := exe_reg_inst
   my_commit_memvalid := io.dmem.req.valid
   my_commit_memaddr  := io.dmem.req.bits.addr
   my_commit_wen      := exe_wben && exe_wbaddr =/= 0.U
   my_commit_data     := exe_wbdata
   ...
   val my_commit_rs1 = Wire(UInt())
   val my_commit_rs2 = Wire(UInt())
   val my_commit_rd  = Wire(UInt())

   my_commit_rs1 := exe_rs1_addr
   my_commit_rs2 := exe_rs2_addr
   my_commit_rd  := exe_wbaddr

   dontTouch(my_commit_rs1)
   dontTouch(my_commit_rs2)
   dontTouch(my_commit_rd)
}
