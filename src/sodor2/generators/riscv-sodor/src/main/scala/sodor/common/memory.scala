...
class MemoryModule(numBytes: Int, useAsync: Boolean)(implicit val conf: SodorCoreParams) {
   ...
   def read(addr: UInt, size: UInt, signed: Bool) = {
      ...
      my_mem_addr := memreader.io.mem_addr
      memreader.io.mem_data := mem.read(my_mem_addr)
      if (conf.markFor=="auto_sandbox") {
         iftConfig(my_mem_addr, "Refined")  // 1-1
      }
   ...
   def write(addr: UInt, data: UInt, size: UInt, en: Bool) = {
      ...
      val my_mem_write_en    = Wire(Bool())
      val my_mem_write_addr  = Wire(UInt())
      val my_mem_write_masks = Wire(Vec(4, Bool()))
      my_mem_write_en    := memwriter.io.en
      my_mem_write_addr  := memwriter.io.mem_addr
      my_mem_write_masks := memwriter.io.mem_masks
      when (my_mem_write_en) {
...
class ScratchPadMemoryBase(num_core_ports: Int, num_bytes: Int = (1 << 21), useAsync: Boolean = true)(implicit val conf: SodorCoreParams) extends Module
{
   ...
   val async_data = new MemoryModule(num_bytes, useAsync)(conf)
