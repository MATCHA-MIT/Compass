...
import chisel3.experimental.circuitIFTConfig
  ...
  extends AbstractInternalTile(coreCtor.nMemPorts)
{
  ...
  if (conf.markFor=="naive") {
    circuitIFTConfig(this, "Naive, PerWord")
  }
  else if (conf.markFor=="cellift") {
    circuitIFTConfig(this, "Refined, PerBit")
  }
  else if (conf.markFor=="auto_sandbox") {
  ...
  core.io := DontCare
  val memory = Module(new AsyncScratchPadMemory(num_core_ports = coreCtor.nMemPorts, num_bytes=conf.mem_size))
