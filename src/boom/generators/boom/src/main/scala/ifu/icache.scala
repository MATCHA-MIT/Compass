...
class ICache(
  ...
  val markFor: String = "None")(implicit p: Parameters)
  extends LazyModule
{
  ...
  with HasBoomFrontendParameters
{
  ...
  val my_icache_hit = Wire(Bool())
  my_icache_hit := Mux(s1_valid, s1_hit, true.B)
  dontTouch(my_icache_hit)
  if (outer.markFor=="auto_sandbox") {
    iftConfig(this, "Naive, PerWord")  // 1-37
    iftConfig(s2_miss, "Refined")  // 1-37
    iftConfig(refill_valid, "Refined")  // 1-37
    iftConfig(s2_hit, "Refined")  // 1-58
    iftConfig(s2_valid, "Refined")  // 1-72
    iftConfig(io.s2_kill, "Refined")  // 1-92
    iftConfig(s1_valid, "Refined")  // 1-100
  }
