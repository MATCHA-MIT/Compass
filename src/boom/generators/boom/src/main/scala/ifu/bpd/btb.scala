...
class BTBBranchPredictorBank(params: BoomBTBParams = BoomBTBParams())(implicit p: Parameters) extends BranchPredictorBank()(p)
{
  ...
  if (boomParams.markFor=="auto_sandbox") {
    iftConfig(this, "Naive, PerWord")  // 1-65
    iftConfig(doing_reset, "Refined")  // 1-83
    iftConfig(s1_valid, "Refined")  // 1-83
    for (i <- 0 until bankWidth) {
      iftConfig(s1_resp(i).valid, "Refined")  // 1-94
      iftConfig(s1_hits(i), "Refined")  // 1-95  // TODO: After adding this, become slow
    }
    iftConfig(s1_is_jal, "Refined")  // 1-95
  }
