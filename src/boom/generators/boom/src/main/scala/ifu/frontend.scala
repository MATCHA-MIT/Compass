 ...
 */
class BoomFrontend(val icacheParams: ICacheParams, staticIdForMetadataUseOnly: Int, markFor: String = "None")(implicit p: Parameters) extends LazyModule
{
  lazy val module = new BoomFrontendModule(this)
  val icache = LazyModule(new boom.ifu.ICache(icacheParams, staticIdForMetadataUseOnly, markFor))
  ...
  with HasBoomFrontendParameters
{
  ...
  val tlb = Module(new TLB(true, log2Ceil(fetchBytes), TLBConfig(nTLBSets, nTLBWays), boomParams.markFor))
  io.ptw <> tlb.io.ptw
  ...
  val my_icache_io_resp_valid = WireInit(icache.io.resp.valid)

  when ((s2_valid && !my_icache_io_resp_valid) ||
        (s2_valid && my_icache_io_resp_valid && !f3_ready)) {
    s0_valid := (!s2_tlb_resp.ae.inst && !s2_tlb_resp.pf.inst) || s2_is_replay || s2_tlb_miss
    s0_vpc   := s2_vpc
    s0_is_replay := s2_valid && my_icache_io_resp_valid
  ...
  for (b <- 0 until nBanks) {
    ...
    for (w <- 0 until bankWidth) {
      ...
      if (boomParams.markFor=="auto_sandbox") {
        iftConfig(brsigs.cfi_type, "Refined")  // 1-99
      }
  ...
  if (boomParams.markFor=="auto_sandbox") {
    iftConfig(this, "Naive, PerWord")  // 1-41
    iftConfig(io.cpu.sfence.valid, "Refined")  // 1-42
    iftConfig(io.cpu.redirect_flush, "Refined")  // 1-54
    iftConfig(s2_valid, "Refined")  // 1-60
    iftConfig(my_icache_io_resp_valid, "Refined")  // 1-60
    iftConfig(f3_ready, "Refined")  // 1-60
    iftConfig(f3.io.deq.valid, "Refined")  // 1-60
    iftConfig(f4.io.enq.ready, "Refined")  // 1-60
    iftConfig(s1_valid, "Refined")  // 1-60
    iftConfig(f3_predicted_target, "Refined")  // 1-60
    iftConfig(s1_vpc, "Refined")  // 1-60
    iftConfig(s2_vpc, "Refined")  // 1-60
    iftConfig(f3_correct_f1_ghist, "Refined")  // 1-60
    iftConfig(f3_correct_f2_ghist, "Refined")  // 1-60
    iftConfig(f4, "Naive, PerWord")  // 1-75
    iftConfig(s1_tlb_miss, "Refined")  // 1-76
    iftConfig(s0_valid, "Refined")  // 1-76
    iftConfig(f2_predicted_target, "Refined")  // 1-76
    iftConfig(f2_correct_f1_ghist, "Refined")  // 1-76
    iftConfig(f3, "Refined, PerWord")  // 1-77, 1-81 Naive->Refined
    iftConfig(s2_xcpt, "Refined")  // 1-80
    iftConfig(f3_bpd_resp, "Refined, PerWord")  // 1-88, 1-89 Naive->Refined
    iftConfig(icache.io.resp.valid, "Refined")  // 1-93
    iftConfig(s2_tlb_resp.ae.inst, "Refined")  // 1-93
    iftConfig(s2_tlb_resp.pf.inst, "Refined")  // 1-93
    iftConfig(s2_tlb_miss, "Refined")  // 1-93
    iftConfig(f4_btb_corrections, "Naive, PerWord")  // 1-96
  }
