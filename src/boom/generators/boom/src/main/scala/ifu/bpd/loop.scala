...
class LoopBranchPredictorBank(implicit p: Parameters) extends BranchPredictorBank()(p)
{
  ...
  class LoopBranchPredictorColumn extends Module {
    ...
    if (boomParams.markFor=="auto_sandbox") {
      iftConfig(f3_entry.conf, "Refined")  // 1-66
      iftConfig(this, "Naive, PerWord")  // 1-87
      iftConfig(io.update_repair, "Refined")  // 1-97
      iftConfig(doing_reset, "Refined")  // 1-97
      iftConfig(io.update_mispredict, "Refined")  // 1-98
      iftConfig(tag_match, "Refined")  // 1-98
      iftConfig(f4_fire, "Refined")  // 1-98
      iftConfig(io.update_idx, "Refined")  // 1-98
      iftConfig(f4_idx, "Refined")  // 1-98
    }
  ...
  if (boomParams.markFor=="auto_sandbox") {
    iftConfig(this, "Naive, PerWord")  // 1-86
  }
