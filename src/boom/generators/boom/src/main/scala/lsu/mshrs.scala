  ...
  with HasL1HellaCacheParameters
{
  ...
  if (boomParams.markFor=="auto_sandbox") {
    iftConfig(state, "Refined")  // 1-19
    iftConfig(this, "Naive, PerWord")  // 1-19
    iftConfig(io.req.tag_match, "Refined")  // 10-6
  }
  ...
  with HasL1HellaCacheParameters
{
  ...
  if (boomParams.markFor=="auto_sandbox") {
    iftConfig(io.req(0).valid, "Refined")  // 1-19
    iftConfig(pri_val, "Refined")  // 1-19
  }
