...
class BoomWritebackUnit(markFor: String = "None")(implicit edge: TLEdgeOut, p: Parameters) extends L1HellaCacheModule()(p) {
  ...
  if (markFor=="auto_sandbox") {
    iftConfig(this, "Naive, PerWord")  // 1-20
    iftConfig(state, "Refined")  // 10-4
  }
...
class BoomProbeUnit(markFor: String = "None")(implicit edge: TLEdgeOut, p: Parameters) extends L1HellaCacheModule()(p) {
  val io = IO(new Bundle {
...
class BoomDuplicatedDataArray(implicit p: Parameters) extends AbstractBoomDataArray
{
  ...
  if (boomParams.markFor=="auto_sandbox") {
    iftConfig(this, "Naive, PerWord")
    iftConfig(waddr, "Refined")  // 8-2
    iftConfig(io.write.bits.wmask, "Refined")  // 8-2
    iftConfig(io.write.bits.way_en, "Refined")  // 8-2
    iftConfig(io.write.valid, "Refined")  // 8-2
  }
  ...
  with HasBoomCoreParameters
{
  ...
  val wb = Module(new BoomWritebackUnit(markFor=boomParams.markFor))
  val prober = Module(new BoomProbeUnit(markFor=boomParams.markFor))
  ...
  for (w <- 0 until memWidth) {
    ...
    cache_resp(w).bits.addr     := s2_req(w).addr
  ...
  val my_dcache_hit = Wire(Bool())
  my_dcache_hit := Mux(s2_valid(0), s2_hit(0), true.B)
  dontTouch(my_dcache_hit)




  if (boomParams.markFor=="auto_sandbox") {
    iftConfig(this, "Naive, PerWord")  // 1-12
    iftConfig(s1_valid, "Refined")  // 1-38
    iftConfig(s2_valid(0), "Refined")  // 1-73
    iftConfig(s2_hit(0), "Refined")  // 1-73
    for (w <- 0 until memWidth) {
      iftConfig(s2_req(w).uop.br_mask, "Refined")  // 3-14
      iftConfig(cache_resp(w).valid, "Refined")  // 3-14
    }
