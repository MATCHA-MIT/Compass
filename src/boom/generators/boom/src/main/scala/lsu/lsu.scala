...
import chisel3.experimental.iftConfig
  ...
  with rocket.HasL1HellaCacheParameters
{
  ...
  for (w <- 0 until coreWidth)
  {
    ...
    if (boomParams.markFor=="auto_sandbox") {
      iftConfig(dis_ld_val, "Refined")  // 1-10
      iftConfig(dis_st_val, "Refined")  // 1-10
    }
 ...
 // Prioritize emptying the store queue when it is almost full
  ...
  val can_fire_load_retry    = widthMap(w =>
                               ( ldq_retry_e.valid                            &&
                                ...
                                (if (boomParams.defense=="DelayAll")
                                   io.core.rob_head_idx===ldq_retry_e.bits.uop.rob_idx
                                 else
                                   true.B
                                ) &&
  ...
  for (w <- 0 until memWidth) {
    ...
    def lsu_sched(can_fire: Bool, uses_tlb:Boolean, uses_dc:Boolean, uses_lcam: Boolean, uses_rob:Boolean): Bool = {
      ...
      if (boomParams.markFor=="auto_sandbox") {
        iftConfig(will_fire, "Refined")  // 1-4
      }
    ...
    if (boomParams.markFor=="auto_sandbox") {
      iftConfig(can_fire_load_retry(w), "Refined")  // 1-4
      iftConfig(can_fire_load_wakeup(w), "Refined")  // 1-6
      iftConfig(can_fire_store_commit(w), "Refined")  // 1-22
    }
  ...
  val delay = widthMap(w =>
    if (boomParams.defense=="DelayAll")
      Mux(will_fire_load_incoming(w), io.core.rob_head_idx=/=exe_req(w).bits.uop.rob_idx, false.B)
    else
      false.B)
  val exe_tlb_miss  = widthMap(w => dtlb.io.req(w).valid && (dtlb.io.resp(w).miss || !dtlb.io.req(w).ready) || delay(w))
  ...
  for (i <- 0 until numLdqEntries) {
    ...
    for (w <- 0 until memWidth) {
      ...
      when (do_release_search(w) &&
        ...
        if (boomParams.markFor=="auto_sandbox") {
          iftConfig(forwarded_is_older, "Refined")  // 1-56
        }
        ...
        if (boomParams.markFor=="auto_sandbox") {
          iftConfig(searcher_is_older, "Refined")  // 1-56
        }
    ...
    if (boomParams.markFor=="auto_sandbox") {
      iftConfig(dword_addr_matches, "Refined")  // 1-51
      iftConfig(mask_overlap, "Refined")  // 1-55
      iftConfig(l_is_forwarding, "Refined")  // 1-56
      iftConfig(l_forward_stq_idx, "Refined")  // 1-56
    }
  ...
  for (i <- 0 until numStqEntries) {
    ...
    if (boomParams.markFor=="auto_sandbox") {
      iftConfig(write_mask, "Refined")  // 1-51
      iftConfig(s_uop.is_fence, "Refined")  // 1-51
      iftConfig(s_uop.is_amo, "Refined")  // 1-51
      iftConfig(dword_addr_matches, "Refined")  // 1-51
    }
  ...
  for (w <- 0 until coreWidth)
  {
    ...
    if (boomParams.markFor=="auto_sandbox") {
      iftConfig(commit_store, "Refined")  // 1-10
      iftConfig(idx, "Refined")  // 1-10
    }
  ...
  assert(memWidth==1)
  val my_ROB_memaddr = Mem(numRobRows, UInt(40.W))
  // OPT: Collect addr when memory return to avoid functional consistency.
  when (io.dmem.resp(0).valid) {
    my_ROB_memaddr(io.dmem.resp(0).bits.uop.rob_idx) := io.dmem.resp(0).bits.addr
  }
  // when (io.core.exe(0).req.valid) {
  //   my_ROB_memaddr(io.core.exe(0).req.bits.uop.rob_idx) := io.core.exe(0).req.bits.addr
  // }
  val my_dummy = Wire(UInt())
  dontTouch(my_dummy)
  my_dummy := my_ROB_memaddr(io.core.exe(0).req.bits.uop.rob_idx)




  if (boomParams.markFor=="auto_sandbox") {
    iftConfig(this, "Naive, PerWord")  // 1-2
    iftConfig(ldq_retry_idx, "Refined")  // 1-5
    for (i <- 0 until numLdqEntries) {
      iftConfig(ldq(i).bits.order_fail, "Refined")  // 1-5
    }
    iftConfig(ldq_wakeup_idx, "Refined")  // 1-7
    for (i <- 0 until numLdqEntries) {
      iftConfig(ldq(i).bits.addr.valid, "Refined")  // 1-7
      iftConfig(ldq(i).bits.addr_is_virtual, "Refined")  // 1-7
      iftConfig(ldq(i).bits.executed, "Refined")  // 1-7
      iftConfig(ldq(i).bits.succeeded, "Refined")  // 1-7
      iftConfig(ldq(i).valid, "Refined")  // 1-7
      iftConfig(p1_block_load_mask(i), "Refined")  // 1-7
      iftConfig(p2_block_load_mask(i), "Refined")  // 1-7
      iftConfig(ldq(i).bits.order_fail, "Refined")  // 1-7
    }
    iftConfig(store_needs_order, "Refined")  // 1-9
    iftConfig(block_load_wakeup, "Refined")  // 1-9
    iftConfig(stq_tail, "Refined")  // 1-10
    for (i <- 0 until numStqEntries) {
      iftConfig(stq(i).bits.committed, "Refined")  // 1-10
    }
    iftConfig(clear_store, "Refined")  // 1-10
    iftConfig(stq_head, "Refined")  // 1-10
    for (i <- 0 until numStqEntries) {
      iftConfig(stq(i).bits.uop.is_fence, "Refined")  // 1-11
    }
    iftConfig(hella_state, "Refined")  // 1-14
    iftConfig(stq_retry_idx, "Refined")  // 1-15
    for (i <- 0 until numStqEntries) {
      iftConfig(stq(i).valid, "Refined")  // 1-15
      iftConfig(stq(i).bits.addr.valid, "Refined")  // 1-15
      iftConfig(stq(i).bits.addr_is_virtual, "Refined")  // 1-15
    }
    iftConfig(dtlb.io.miss_rdy, "Refined")  // 1-18
    iftConfig(io.core.rob_head_idx, "Refined")  // 1-18
    for (i <- 0 until numLdqEntries) {
      iftConfig(ldq(i).bits.uop.rob_idx, "Refined")  // 1-18
    }
    iftConfig(lcam_mask, "Refined")  // 1-51
    iftConfig(ldq_tail, "Refined")  // 1-53
    iftConfig(do_release_search, "Refined")  // 1-55
    for (i <- 0 until numLdqEntries) {
      iftConfig(ldq(i).bits.st_dep_mask, "Refined")  // 1-56
      iftConfig(ldq(i).bits.forward_std_val, "Refined")  // 1-56
      iftConfig(ldq(i).bits.observed, "Refined")  // 1-56
    }
    iftConfig(lcam_stq_idx, "Refined")  // 1-56
    iftConfig(do_st_search, "Refined")  // 1-56
    iftConfig(lcam_addr, "Refined")  // 1-56
    iftConfig(s1_executing_loads, "Refined")  // 1-56
    iftConfig(do_ld_search, "Refined")  // 1-56
    for (i <- 0 until numStqEntries) {
      iftConfig(stq(i).bits.data.valid, "Refined")  // 1-59
    }
    iftConfig(io.core.fp_stdata.valid, "Refined")  // 1-61
    iftConfig(io.core.exception, "Refined")  // 1-90
    iftConfig(reset, "Refined")  // 1-90
    iftConfig(stdf_clr_bsy_brmask, "Refined")  // 3-9
    iftConfig(clr_bsy_brmask, "Refined")  // 3-10
    iftConfig(reset, "Refined")  // 5-1
    for (i <- 0 until numStqEntries) {
      iftConfig(stq(i).bits.succeeded, "Refined")  // 5-1
    }
    iftConfig(wb_forward_ldq_idx, "Refined")  // 6-1
    for (i <- 0 until numLdqEntries) {
      iftConfig(ldq(i).bits.uop.br_mask, "Refined")  // 6-1
    }
    iftConfig(exe_tlb_miss, "Refined")  // 10-1
    for (w <- 0 until memWidth) {
