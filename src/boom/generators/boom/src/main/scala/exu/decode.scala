...
//     cs.div && !csr.io.status.isa('m'-'a') || TODO check for illegal div instructions
  ...
  my_csr_cmd := cs.csr_cmd
  dontTouch(my_csr_cmd)
  if (boomParams.markFor=="auto_sandbox") {
    iftConfig(io.interrupt, "Refined")  // 1-33
    iftConfig(io.enq.uop.is_sfb, "Refined")  // 1-33
    iftConfig(uop.bp_debug_if, "Refined")  // 1-33
    iftConfig(uop.bp_xcpt_if, "Refined")  // 1-33
    iftConfig(uop.xcpt_pf_if, "Refined")  // 1-33
    iftConfig(uop.xcpt_ae_if, "Refined")  // 1-33
    iftConfig(id_illegal_insn, "Refined")  // 1-33
    iftConfig(sfence, "Refined")  // 1-36
    iftConfig(system_insn, "Refined")  // 1-36
    iftConfig(csr_en, "Refined")  // 1-36
    iftConfig(cs.fp_val, "Refined")  // 1-36
    iftConfig(cs.fp_single, "Refined")  // 1-36
    iftConfig(cs.is_amo, "Refined")  // 1-36
  }
