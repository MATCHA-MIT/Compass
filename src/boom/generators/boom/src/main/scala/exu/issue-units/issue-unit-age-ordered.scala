  ...
  extends IssueUnit(params.numEntries, params.issueWidth, numWakeupPorts, params.iqType, params.dispatchWidth)
{
  ...
  if (boomParams.markFor=="auto_sandbox") {
    for (i <- 0 until numIssueSlots) {
      iftConfig(requests(i), "Refined")  // 1-52
      iftConfig(issue_slots(i).uop.fu_code, "Refined")  // 1-52
    }
    for (w <- 0 until issueWidth) {
      iftConfig(io.fu_types(w), "Refined")  // 1-52
    }
    for (i <- 0 until dispatchWidth) {
      iftConfig(io.dis_uops(i).valid, "Refined")  // 1-70
      iftConfig(dis_uops(i).exception, "Refined")  // 1-70
      iftConfig(dis_uops(i).is_fence, "Refined")  // 1-70
      iftConfig(dis_uops(i).is_fencei, "Refined")  // 1-70
    }
  }
