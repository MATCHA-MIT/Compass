  ...
  with tile.HasFPUParameters
{
  ...
  val divsqrt = Module(new hardfloat.DivSqrtRecF64(boomParams.markFor))
  ...
  if (boomParams.markFor=="auto_sandbox") {
    iftConfig(this, "Naive PerWord")  // 2-2
    iftConfig(io.req.valid, "Refined")  // 3-7
    iftConfig(io.brupdate.b1.mispredict_mask, "Refined")  // 3-7
    iftConfig(io.req.bits.uop.br_mask, "Refined")  // 3-7
    iftConfig(io.req.bits.kill, "Refined")  // 3-7
  }
