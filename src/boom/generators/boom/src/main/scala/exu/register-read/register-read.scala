...
)(implicit p: Parameters) extends BoomModule
{
  ...
  for (w <- 0 until issueWidth) {
    ...
    for (b <- 0 until numTotalBypassPorts)
    {
      ...
      if (boomParams.markFor=="auto_sandbox") {
        iftConfig(bypass.valid, "Refined")  // 6-4
        iftConfig(bypass.bits.uop.pdst, "Refined")  // 6-4
        iftConfig(bypass.bits.uop.rf_wen, "Refined")  // 6-4
        iftConfig(bypass.bits.uop.dst_rtype, "Refined")  // 6-4
      }
    }
    if (boomParams.markFor=="auto_sandbox") {
      iftConfig(prs1, "Refined")  // 6-4
      iftConfig(lrs1_rtype, "Refined")  // 6-4
      iftConfig(prs2, "Refined")  // 6-4
      iftConfig(lrs2_rtype, "Refined")  // 6-4
    }
  ...
  if (boomParams.markFor=="auto_sandbox") {
    iftConfig(this, "Naive, PerWord")  // 1-3
    for (w <- 0 until issueWidth) {
      iftConfig(rrd_uops(w).br_mask, "Refined")  // 6-2
      iftConfig(io.iss_uops(w).prs1, "Refined")  // 6-3
      iftConfig(io.iss_uops(w).prs2, "Refined")  // 6-4
    }
  }
