    ...
    needsFcsr = needsFcsr)
{
  ...
  if (numStages > 0) {
    ...
    if (markFor=="auto_sandbox") {
      for (i <- 0 until numStages) {
        iftConfig(r_uops(i).br_mask, "Refined")  // 3-13
      }
    }
  ...
  if (markFor=="auto_sandbox") {
    iftConfig(io.req.bits.uop.br_mask, "Refined")  // 3-8
  }
 ...
 * @param dataWidth width of the data being operated on in the functional unit
 */
class ALUUnit(isJmpUnit: Boolean = false, numStages: Int = 1, dataWidth: Int, markFor: String = "None")(implicit p: Parameters)
 ...
 */
class MemAddrCalcUnit(markFor: String = "None")(implicit p: Parameters)
  extends PipelinedFunctionalUnit(
    numStages = 0,
    numBypassStages = 0,
    earliestBypassStage = 0,
 ...
 */
class FPUUnit(markFor: String = "None")(implicit p: Parameters)
  extends PipelinedFunctionalUnit(
 ...
 */
class IntToFPUnit(latency: Int, markFor: String = "None")(implicit p: Parameters)
  extends PipelinedFunctionalUnit(
    numStages = latency,
    numBypassStages = 0,
    earliestBypassStage = 0,
    ...
    dataWidth = dataWidth)
{
  ...
  if (boomParams.markFor=="auto_sandbox") {
    iftConfig(io.brupdate.b1.mispredict_mask, "Refined")  // 1-74
    iftConfig(io.req.valid, "Refined")  // 1-74
  }
 ...
 */
class PipelinedMulUnit(numStages: Int, dataWidth: Int, markFor: String = "None")(implicit p: Parameters)
  extends PipelinedFunctionalUnit(
    numStages = numStages,
    numBypassStages = 0,
    earliestBypassStage = 0,
