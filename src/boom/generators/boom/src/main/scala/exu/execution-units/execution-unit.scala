  ...
  with freechips.rocketchip.rocket.constants.MemoryOpConstants
{
  ...
  if (hasMul) {
    imul = Module(new PipelinedMulUnit(imulLatency, xLen, markFor=boomParams.markFor))
    imul.io <> DontCare
  ...
  if (hasIfpu) {
    ifpu = Module(new IntToFPUnit(latency=intToFpLatency, boomParams.markFor))
    ifpu.io.req        <> io.req
  ...
  if (hasMem) {
    require(!hasAlu)
    val maddrcalc = Module(new MemAddrCalcUnit(markFor = boomParams.markFor))
    ...
    hasFpiu = hasFpiu) with tile.HasFPUParameters
{
  ...
  if (hasFpu) {
    fpu = Module(new FPUUnit(boomParams.markFor))
  ...
  if (boomParams.markFor=="auto_sandbox") {
    iftConfig(io.req.bits.uop.br_mask, "Refined")  // 3-15
  }
