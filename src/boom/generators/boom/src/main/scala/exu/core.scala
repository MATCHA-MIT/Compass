...
//   "CSRFile is being written to spuriously.")
  ...
  if (boomParams.markFor=="auto_sandbox") {
    iftConfig(this, "Naive, PerWord")  // 1-16
    iftConfig(dec_valids, "Refined")  // 1-24
    iftConfig(rob.io.ready, "Refined")  // 3-11
    iftConfig(brupdate.b1.mispredict_mask, "Refined")  // 3-11
    iftConfig(brupdate.b2.mispredict, "Refined")  // 3-11
    iftConfig(io.ifu.redirect_flush, "Refined")  // 3-11
    iftConfig(dis_valids, "Refined")  // 3-11
    for (i <- 0 until coreWidth) {
      iftConfig(ren_stalls(i), "Refined")  // 3-11
      iftConfig(io.lsu.ldq_full(i), "Refined")  // 3-11
      iftConfig(dis_uops(i).uses_ldq, "Refined")  // 3-11
      iftConfig(io.lsu.stq_full(i), "Refined")  // 3-11
      iftConfig(dis_uops(i).uses_stq, "Refined")  // 3-11
      iftConfig(dispatcher.io.ren_uops(i).ready, "Refined")  // 3-11
      iftConfig(wait_for_empty_pipeline(i), "Refined")  // 3-11
      iftConfig(wait_for_rocc(i), "Refined")  // 3-11
