  ...
  )(implicit p: Parameters) extends BoomModule
{
  ...
  assert(retireWidth==1)

  val my_ROB_inst = Mem(numRobRows, UInt(32.W))
  when (io.enq_valids(0)) { my_ROB_inst(rob_tail) := io.enq_uops(0).debug_inst }

  val my_commit_ld = Wire(Bool())
  val my_commit_valid = Wire(Bool())
  val my_commit_ismem = Wire(Bool())
  val my_commit_inst  = Wire(UInt())
  val my_commit_data  = Wire(UInt())
  val my_ROB_tail     = Wire(UInt())
  val my_ROB_head     = Wire(UInt())
  my_commit_ld := io.commit.valids(0) && io.commit.uops(0).uses_ldq
  my_commit_valid := io.commit.valids(0)
  my_commit_ismem := io.commit.uops(0).uses_ldq || io.commit.uops(0).uses_stq
  my_commit_inst  := my_ROB_inst(rob_head)
  my_commit_data  := io.commit.debug_wdata(0)
  my_ROB_tail     := rob_tail
  my_ROB_head     := rob_head
  dontTouch(my_commit_ld)
  dontTouch(my_commit_valid)
  dontTouch(my_commit_ismem)
  dontTouch(my_commit_inst)
  dontTouch(my_commit_data)
  dontTouch(my_ROB_tail)
  dontTouch(my_ROB_head)


  // OPT2: Consider head of ROB will always commit.
  val can_ISATaint_commit = Wire(Bool())
  val can_ISATaint_commit_but_not_commit = RegInit(Bool(), false.B)
  val will_ISATaint_commit = Wire(Bool())
  can_ISATaint_commit := rob_head_vals(0)
  can_ISATaint_commit_but_not_commit := can_ISATaint_commit && !io.commit.valids(0)
  will_ISATaint_commit := can_ISATaint_commit && !can_ISATaint_commit_but_not_commit
  dontTouch(will_ISATaint_commit)
...
//                  "FP instruction writing back exc bits is overriding an existing exception.")
    ...
    if (boomParams.markFor=="auto_sandbox") {
      iftConfig(rob_val, "Refined")  // 1-8
      iftConfig(rob_bsy, "Refined")  // 1-17
      iftConfig(rbk_row, "Refined")  // 1-46
      iftConfig(rob_exception, "Refined")  // 1-46
    }
  ...
  val my_ROB_squash      = Wire(Bool())
  val my_ROB_squash_tail = Wire(UInt())
  my_ROB_squash      := false.B
  my_ROB_squash_tail := 0.U
  dontTouch(my_ROB_squash)
  dontTouch(my_ROB_squash_tail)

  when (rob_state === s_rollback && (rob_tail =/= rob_head || maybe_full)) {
    my_ROB_squash      := true.B
    my_ROB_squash_tail := WrapDec(rob_tail, numRobRows)
  } .elsewhen (rob_state === s_rollback && (rob_tail === rob_head) && !maybe_full) {
  } .elsewhen (io.brupdate.b2.mispredict) {
    my_ROB_squash      := true.B
    my_ROB_squash_tail := WrapInc(GetRowIdx(io.brupdate.b2.uop.rob_idx), numRobRows)
  }
  ...
  if (boomParams.markFor=="auto_sandbox") {
    iftConfig(this, "Naive, PerWord")  // 1-1
    iftConfig(rob_head, "Refined")  // 1-8
    iftConfig(io.csr_stall, "Refined")  // 1-27
    iftConfig(can_throw_exception, "Refined")  // 1-27
    iftConfig(exception_thrown, "Refined")  // 1-28
    iftConfig(rob_state, "Refined")  // 1-28
    iftConfig(r_xcpt_uop.rob_idx, "Refined")  // 1-28
    iftConfig(r_xcpt_val, "Refined")  // 1-28
    iftConfig(io.lxcpt.bits.uop.rob_idx, "Refined")  // 1-28
    iftConfig(io.enq_valids(0), "Refined")  // 1-28
    iftConfig(io.enq_uops(0).exception, "Refined")  // 1-28
    iftConfig(enq_xcpts(0), "Refined")  // 1-28
    iftConfig(io.lxcpt.valid, "Refined")  // 1-28
    iftConfig(io.brupdate.b1.mispredict_mask, "Refined")  // 1-28
    iftConfig(flush_val, "Refined")  // 1-28
