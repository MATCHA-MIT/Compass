  ...
  (implicit p: Parameters) extends BoomModule
{
  ...
  for (w <- 0 until plWidth) {
    ...
    if (boomParams.markFor=="auto_sandbox") {
      iftConfig(r_valid, "Refined")  // 1-25
    }
...
(implicit p: Parameters) extends AbstractRenameStage(plWidth, numPhysRegs, numWbPorts)(p)
{
  ...
  if (boomParams.markFor=="auto_sandbox") {
    iftConfig(io.dis_ready, "Refined")  // 1-25
    iftConfig(io.dec_fire, "Refined")  // 1-25
    iftConfig(io.dis_fire, "Refined")  // 1-25
    iftConfig(io.kill, "Refined")  // 1-25
    iftConfig(this, "Naive, PerWord")  // 1-26
  }
