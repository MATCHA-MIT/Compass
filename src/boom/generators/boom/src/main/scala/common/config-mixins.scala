...
class WithMarkFor(s: String = "None") extends Config((site, here, up) => {
  case TilesLocated(InSubsystem) => up(TilesLocated(InSubsystem), site) map {
    case tp: BoomTileAttachParams => tp.copy(tileParams = tp.tileParams.copy(
      core = tp.tileParams.core.copy(markFor = s)
    ))
    case other => other
  }
})

class WithDefense(s: String = "None") extends Config((site, here, up) => {
  case TilesLocated(InSubsystem) => up(TilesLocated(InSubsystem), site) map {
    case tp: BoomTileAttachParams => tp.copy(tileParams = tp.tileParams.copy(
      core = tp.tileParams.core.copy(defense = s)
    ))
    case other => other
  }
})

class MyBoomBaseConfig(n: Int = 1) extends Config(
      ...
      (0 until n).map { i =>
        BoomTileAttachParams(
          tileParams = BoomTileParams(
            ...
            // NOTE: `nSets=1, nWays=1` does not work, minimal is to have 2 sets
            dcache = Some(
              DCacheParams(rowBits = 64, nSets=2, nWays=1, nMSHRs=2, nTLBWays=8)
            ),
            icache = Some(
              ICacheParams(rowBits = 64, nSets=2, nWays=1, fetchBytes=2*4)
            ),
            tileId = i + idOffset
          ),
