...
import chisel3.experimental.{circuitIFTConfig, iftConfig}
  ...
  with SourcesExternalNotifications
{
  ...
  // Frontend/ICache
  val frontend = LazyModule(new BoomFrontend(tileParams.icache.get, tileId, boomParams.core.markFor))
...
class BoomTileModuleImp(outer: BoomTile) extends BaseTileModuleImp(outer){
  ...
  if (outer.boomParams.core.markFor=="naive") {
    circuitIFTConfig(this, "Naive, PerWord")
  }
  else if (outer.boomParams.core.markFor=="cellift") {
    circuitIFTConfig(this, "Refined, PerBit")
  }
  else if (outer.boomParams.core.markFor=="auto_sandbox") {
    circuitIFTConfig(this, "Naive, PerModule")
  }
  ...
  // PTW
  val ptw  = Module(new PTW(ptwPorts.length, markFor=outer.boomParams.core.markFor)
                           (outer.dcache.node.edges.out(0), outer.p))
  ...
  val my_mem_valid = Wire(Bool())
  val my_mem_addr = Wire(UInt())
  my_mem_valid := lsu.io.dmem.req.bits(0).valid
  my_mem_addr := Mux(my_mem_valid, lsu.io.dmem.req.bits(0).bits.addr, 0.U)
  dontTouch(my_mem_valid)
  dontTouch(my_mem_addr)
