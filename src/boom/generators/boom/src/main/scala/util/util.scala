  ...
  with boom.common.HasBoomCoreParameters
{
  ...
  val my_io_empty = WireInit(io.empty)
  ...
  val do_deq = WireInit((io.deq.ready || !valids(deq_ptr.value)) && !my_io_empty)
  ...
  if (boomParams.markFor=="auto_sandbox") {
    iftConfig(this, "Naive, PerWord")  // 1-39
    iftConfig(my_io_empty, "Refined")  // 1-91
    iftConfig(out.uop.br_mask, "Refined")  // 3-12
  }
