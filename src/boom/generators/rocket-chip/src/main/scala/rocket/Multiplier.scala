...
class MulDiv(cfg: MulDivParams, width: Int, nXpr: Int = 32, aluFn: ALUFN = new ALUFN, markFor: String = "None") extends Module {
  ...
  if (markFor=="auto_sandbox") {
    iftConfig(io.req.bits.fn, "Refined")  // 1-34
    iftConfig(this, "Naive, PerWord")  // 1-35
    iftConfig(state, "Refined")  // 2-1
    iftConfig(count, "Refined")  // 2-1
    iftConfig(io.req.valid, "Refined")  // 4-1
  }
