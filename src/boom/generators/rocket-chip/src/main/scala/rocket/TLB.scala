...
/** =Overview=
  ...
  */
class TLB(instruction: Boolean, lgMaxSize: Int, cfg: TLBConfig, markFor: String = "None")(implicit edge: TLEdgeOut, p: Parameters) extends CoreModule()(p) {
  ...
  val pmp = Module(new PMPChecker(lgMaxSize, markFor))
  ...
  val hitsVec = VecInit(all_entries.map(e => vm_enabled && e.hit(vpn, priv_v)))
  ...
  if (markFor=="auto_sandbox") {
    iftConfig(this, "Naive, PerWord")  // 1-31
    iftConfig(io.ptw.resp.valid, "Refined")  // 1-43
    iftConfig(vm_enabled, "Refined")  // 1-49
    iftConfig(vsatp_mode_mismatch, "Refined")  // 1-49
    iftConfig(bad_va, "Refined")  // 1-49
    iftConfig(hitsVec, "Refined")  // 1-50
    iftConfig(special_entry.get.valid, "Refined")  // 1-50
    iftConfig(io.ptw.resp.bits.homogeneous, "Refined")  // 1-67
  }
