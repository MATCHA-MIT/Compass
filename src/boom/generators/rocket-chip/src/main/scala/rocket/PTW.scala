...
/** PTW contains L2TLB, and performs page table walk for high level TLB, and cache queries from L1 TLBs(I$, D$, RoCC)
  ...
  */
class PTW(n: Int, markFor: String = "None")(implicit edge: TLEdgeOut, p: Parameters) extends CoreModule()(p) {
  ...
  val (l2_hit, l2_error, l2_pte, l2_tlb_ram) = if (coreParams.nL2TLBEntries == 0) (false.B, false.B, WireDefault(0.U.asTypeOf(new PTE)), None) else {
    ...
    if (markFor=="auto_sandbox") {
      iftConfig(s1_valid, "Refined")  // 1-48
    }
  ...
  if (markFor=="auto_sandbox") {
    iftConfig(this, "Naive, PerWord")  // 1-13
    iftConfig(mem_resp_valid, "Refined")  // 1-20
    iftConfig(state, "Refined")  // 1-23
  }
