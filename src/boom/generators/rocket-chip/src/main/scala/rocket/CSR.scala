...
class CSRFile(
  ...
  markFor: String = "None")(implicit p: Parameters)
      ...
      when (decoded_addr(CSRs.satp)) {
        if (usingVM) {
          ...
          if (markFor=="auto_sandbox") {
            iftConfig(new_satp.mode, "Refined")  // 1-45
          }
  ...
  if (markFor=="auto_sandbox") {
    iftConfig(this, "Naive, PerWord")  // 1-44
    iftConfig(csr_wen, "Refined")  // 1-45
    iftConfig(io.rw.addr, "Refined")  // 1-45
    iftConfig(reg_mstatus.v, "Refined")  // 1-45
    iftConfig(io.rw.cmd, "Refined")  // 1-45
    iftConfig(io.rw.wdata, "Refined")  // 1-68
    for (i <- 0 until nPMPs) {
      iftConfig(reg_pmp(i).cfg.a, "Refined")  // 1-69
      iftConfig(reg_pmp(i).cfg.l, "Refined")  // 1-69
    }
  }
