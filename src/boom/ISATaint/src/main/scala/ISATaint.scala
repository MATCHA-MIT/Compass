...
class ISATaintIo extends Bundle {
  ...
  val memaddr = Input(UInt(40.W))
}


class ISATaint(
  memdSize: Int = 8 * 8,
  memdHigherBits: (Int, Int) = (0x2000003, 34)
  // Note: No need to constrain pc becuase as long as you fetch, it must from
  //       the valid region. For memd, no need to contrain for load, but store
  //       will be issued after the instruction commits.
) extends Module {
  ...
  //       itself).
  val simplification_DMEMHit   = Wire(Bool())
  val simplification_legalInst = Wire(Bool())
  simplification_DMEMHit   := true.B
  ...
  simplificationAssumption := simplification_DMEMHit && simplification_legalInst
  ...
  def set_simplification_DMEMHit(addr : UInt) = {
    simplification_DMEMHit := addr(39, 40-memdHigherBits._2)===
                              memdHigherBits._1.U
  }
  ...
  when (io.commit) {
    ...
    .elsewhen (inst===IType("LB")) {
      val addr = io.memaddr
      val addr_taint = rfRead_taint(rs1)
      rfWrite_taint(rd, addr_taint | memd_taint(addr))

      my_commit_memaddr_taint := addr_taint | pc_taint

      set_simplification_DMEMHit(addr)
    }


    .elsewhen (inst===IType("LH")) {
      val addr = io.memaddr
      val addr_taint = rfRead_taint(rs1)
      rfWrite_taint(rd, addr_taint | memd_taint(addr+1.U) | memd_taint(addr))
